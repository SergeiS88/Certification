// принимает типамассив элементов, которым атрибутом присваивается рамка и принимает цвет рамки.
const PassStyles = (elem, col) => {
  elem.forEach(element => {
    element.setAttribute('style', `border: 1px solid ${col};`);
  });
}
// по отжатию клавиши вызываем функцию и сравниваем пароли.
document.querySelector("#password2").addEventListener('keyup', function () {
  PassStyles(document.querySelectorAll('.pass'),
...
// рейтинг. Слушаем изменения в блоке ретинга.
document.querySelector('.rating-area').addEventListener('change', (ev) => {
  const checkedNum = document.querySelector('.form__input:checked').value ?? undefined;
  // пробегаем по инпутам и показываем звёзды какие нужны.
  document.querySelectorAll('.form__input').forEach((item) => {
    const currentLabel = item.previousElementSibling;
    if(item.value <= checkedNum) {
        currentLabel.firstElementChild.classList.remove('none');
        currentLabel.lastElementChild.classList.add('none');
    }
    else{
        currentLabel.firstElementChild.classList.add('none');
        currentLabel.lastElementChild.classList.remove('none');
...
// адаптивная активация слайдера
const resizeElement = new ResizeObserver( entries => {
  for (let entry of entries) {
      const cr = entry.contentRect;
      if (cr.width < 1025) {
          document.querySelector('#slider').classList.add('itc-slider');
          document.querySelector('.slider__btns').classList.remove('none')
